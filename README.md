...
### アクショングループ用アプリケーションの作成

Azure Functionsの呼び出し元をアクショングループに制限するために、Azure FunctionsにEntra ID認証を設定します。

このリポジトリでは、Secure Webhook 用の Entra ID アプリケーション登録も `az deployment` 実行時に Bicep で自動作成します。[こちら](https://techcommunity.microsoft.com/blog/azuregovernanceandmanagementblog/announcing-ga-of-bicep-templates-support-for-microsoft-entra-id-resources/4437163?utm_source=chatgpt.com) で紹介されている Microsoft Graph向けBicep拡張 (`Microsoft.Graph/*`) を使い、以下をまとめてデプロイします。

- テナントに Secure Webhook 用のアプリケーションを登録する
- 対応するサービスプリンシパルを作成する
- アプリケーションに `ActionGroupsSecureWebhook` アプリロールを作成する
- アプリロールを、アクショングループを表すエンタープライズアプリケーション (アプリケーションID: `461e8683-5575-4561-ac7f-899cc907d62a`) に割り当てる
- Azure Functions で Entra ID 認証を有効化し、作成したアプリケーションを割り当てる
- アクショングループに Secure Webhook を登録する

必要に応じて、`webhookAppOwnerObjectId` にアプリケーション所有者として追加したいユーザーまたはサービスプリンシパルのオブジェクト ID を指定できます。未指定でもデプロイは可能です。
...
| webhookAppOwnerObjectId | Secure Webhook 用アプリケーションの所有者にしたいユーザーまたはサービスプリンシパルのオブジェクト ID。不要なら空文字のままで構いません |

ここで設定した変数のうち、アプリケーション登録に関する値は Bicep が使用します。作成されたアプリケーションのクライアント ID とオブジェクト ID はデプロイ出力から確認できます。
